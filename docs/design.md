...
│  GitOps layout    │  pkg/kubernetes + pkg/kubernetes/fluxcd
└────────┬──────────┘
         │
         ▼
...
| Composability | Helm subcharts | Multi-component Application model; optional/multi-instance components and split files deferred to Phase 2 (go-kure/launcher#39); standalone post-generation patching (`pkg/patch`) is a separate, opt-in step |
...
- Component handlers: webservice, worker, deployment, postgresql, cronjob, job, helmchart, daemonset, statefulset (go-kure/launcher#48, go-kure/launcher#343, go-kure/launcher#344)
...
3. **Package distribution** — How kurel packages are published, versioned, and referenced
   by consumers. `kurel build` runs today against a standard local layout without extra
   flags (Phase 3, complete); OCI-based publishing and pulling is Phase 5, gate met but
   not yet started.
