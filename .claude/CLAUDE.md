...
- Enforced in CI via composite actions from `go-kure/.github` (launcher no longer vendors copies): check-doc-sync (structure), check-links (rendered links), and the `doc-gate` job (mapped-package source change must touch its docs; bypass only via the maintainer `docs-skip` label)
