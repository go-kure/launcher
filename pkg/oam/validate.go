...
// lowering rule passes validation even though it is absent from the strict
// allowlists below, because the lowering fixpoint removes it before any handler ever
// sees it. An empty LowerableTypes is exactly today's strict behavior — the two
// wrappers above are unchanged in observable effect.
...
// mergeTypeSets returns a new set containing every key of base plus every element of
// extra, without mutating base.
