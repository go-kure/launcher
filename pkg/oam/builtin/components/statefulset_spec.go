...
func parseStatefulSetUpdateStrategy(raw map[string]any) (*appsv1.StatefulSetUpdateStrategy, error) {
	...
	// isExplicitNull rather than a bare presence check, for the same reason
	// authoredValue exists (see common.go): this leaf is optional in the schema,
	// so pkg/oam accepts an explicit null for it, and parseMaxUnavailable would
	// then reject the nil as a wrong type. parseMaxUnavailable takes an `any`
	// rather than a (map, key) pair, so it cannot route through authoredValue
	// itself and the check stays here.
