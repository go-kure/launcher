...
// exhaustive: it exercises exactly those three fields. The rest of the pod
// spec buildPodSpec projects — the authored pod-level fields, the appended
// volumes and the main, init and sidecar containers — is guarded by
// podspec_render_reuse_aliasing_test.go (go-kure/launcher#425), in the same
// nested-reference style with its own shallow negative control.
//
// Adding a field to the projection without adding a case to one of the two
// files leaves it silently unguarded, which is exactly how the three fields
// above came to be added outside the boundary of the test directly above this
// one.
