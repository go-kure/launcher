...
func parseTolerations(props map[string]any) ([]corev1.Toleration, error) {
	tolList, present, err := parseObjectList(props, "tolerations")
	if err != nil {
		return nil, err
	}
	if !present {
	...
	for i, m := range tolList {
		...
		// different documents — hence parseInt64Field's present flag rather than
		...
		if seconds, present, err := parseInt64Field(m, "tolerationSeconds", indexedLabel("toleration", i)+".tolerationSeconds"); err != nil {
			return nil, err
		} else if present {
