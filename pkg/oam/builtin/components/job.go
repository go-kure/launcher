...
func (c *JobConfig) createJob(app *stack.Application) (*batchv1.Job, error) {
	...
	// After the template is assigned, not before: this checks podFailurePolicy
	// against the restart policy and container names that were actually
	// emitted (go-kure/launcher#345).
	if err := validateJobPodFailurePolicyAgainstTemplate(&job.Spec); err != nil {
		return nil, err
	}
