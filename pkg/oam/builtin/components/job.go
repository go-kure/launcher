...
// EmitsAutoHealthCheck vetoes the auto health check for a suspended job, the
// same shape as deployment's veto for `paused: true`: the document instructs the
// workload not to run, so waiting on it is not a health signal but a guaranteed
// timeout. A job that is not suspended reaches Complete or Failed, which is what
// makes the check meaningful — see componentHealthCheckGVK's note in transform.go.
