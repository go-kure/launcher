...
// component; each maps to the error explaining why. They are refused rather than
// left undeclared so the message says what to do instead of only naming the key —
// and, for podFailurePolicy, because leaving it undeclared refuses nothing:
// validateProperties runs on EMITTED elements only, never on authored documents
// (property_validate.go's scope note), so an authored key this handler does not
// read is silently dropped rather than rejected.
