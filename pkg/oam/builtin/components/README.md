...
- **job** (go-kure/launcher#344, `job.go`) — a run-to-completion workload:
  ...
  **rejected outright** rather than left undeclared. Leaving it undeclared would
  ...
  `validatePropertyValue`, and is what the `optional*` helpers in `common.go`
  exist for. The JobSpec fields shared with `cronjob` still refuse a null,
  because they predate those helpers; migrating them is go-kure/launcher#394.
  The difference is one-directional — a wrapped field accepts a null the
  ...
  block for exactly as long as the document says to stay suspended. An
