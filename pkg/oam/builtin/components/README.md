...
- **job** (go-kure/launcher#344, `job.go`) — a run-to-completion workload:
  ...
  **Compatibility.** On `job` this is additive — the component type is newer
  ...
  `validatePropertyValue`. Since go-kure/launcher#394 this is not special to
  this component: every optional-field helper in `common.go` routes its presence
  check through `authoredValue`, so the JobSpec fields shared with `cronjob`
  read a null the same way, as does every other kind. The change was
